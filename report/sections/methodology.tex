...
The dataset images were sourced from the "Chicken Dataset" Computer Vision Dataset \cite{chicken-dataset}. This dataset contains images of poultry farm environments with bounding box annotations for individual chickens, see Figure \ref{fig:initial_dataset_example}. For this project, point annotations were created at the center of each bounding box to facilitate density map generation. The original dataset contained 147 images with similar condition images (same viewpoint, lighting, and density).

\begin{figure}
    \centering
    \includegraphics[width=0.35\textwidth]{initial-dataset-example.png}
    \caption{Original dataset image with bounding box annotations (not used in this project).}
    \label{fig:initial_dataset_example}
\end{figure}

\subsubsection{Annotation Format}

Images were manually re-annotated using the LabelMe software \cite{labelme_website} with point annotations. Each annotation JSON contains:
...
See Figure \ref{fig:labelme_example} for an example annotation.

\begin{figure}
    \centering
    \includegraphics[width=0.35\textwidth]{labelme-example.png}
    \caption{Example of a LabelMe annotation with point labels for chicken locations.}
    \label{fig:labelme_example}
\end{figure}
...
This ensures $\sum D_{gt} = N$ where $N$ is the annotated chicken count. See Figure \ref{fig:density_example} for a sample density map.
...
\begin{itemize}
    ...
    \item Batch size: 6 (depends on GPU memory)
    \item Epochs: 500
...
\begin{itemize}
    \item Programming language: Python 3.12.3 \cite{python}, inside a virtual environment
    \item Framework: PyTorch 2.0+ \cite{pytorch}
    \item Hardware: CUDA-enabled GPU (NVIDIA GeForce RTX 1650 with 4GB VRAM from NVIDIA Corporation, Nvidia, Santa Clara, CA, USA), with CUDA 12.9
